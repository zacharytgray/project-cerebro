...
const Modal = ({ isOpen, onClose, title, children }: { isOpen: boolean; onClose: () => void; title: string; children: React.ReactNode }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-card text-foreground border border-border rounded-xl w-full max-w-md shadow-2xl overflow-hidden">
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-gradient-to-br from-[#0b0e14] via-[#0d111c] to-[#0a0f1a]' : 'bg-slate-50'} text-foreground`}>
      <div className="flex min-h-screen">
        <aside className={`hidden lg:flex flex-col gap-6 border-r ${isDark ? 'border-white/10 bg-white/5' : 'border-black/10 bg-white/80'} backdrop-blur-xl p-6 shadow-[inset_0_0_40px_rgba(59,130,246,0.05)] transition-all duration-300 ${sidebarOpen ? 'w-64 opacity-100 translate-x-0' : 'w-0 opacity-0 -translate-x-full pointer-events-none'}`}>
          <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col gap-5">
            ...
            {selectedTask.description && (
              <div>
                ...
                <div className="text-sm bg-secondary/70 text-foreground p-3 rounded-lg border border-border leading-relaxed">
                  {selectedTask.description}
                </div>
              </div>
            )}
